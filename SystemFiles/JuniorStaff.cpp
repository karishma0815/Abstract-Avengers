...
bool JuniorStaff::canHandle(const CustomerQuery &query) const{
    if (query.type == CustomerQuery::GENERAL) {
        return query.question.find("opening hours") != std::string::npos || query.question.find("location") != std::string::npos || query.question.find("gift wrapping") != std::string::npos;
    } else if (query.type == CustomerQuery::PRICING) {
        return query.question.find("rose") != std::string::npos || query.question.find("orchid") != std::string::npos || query.question.find("student discount") != std::string::npos;
    }
    return false;
}
